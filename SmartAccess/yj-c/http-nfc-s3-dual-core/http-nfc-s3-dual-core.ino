...
void playTone(uint16_t frequency, uint16_t duration_ms) {
  const uint16_t amplitude = 5000;  // 音量（0-32767）
  const uint32_t sample_rate = 44100;
  const uint32_t samples = (duration_ms * sample_rate) / 1000;
  const uint32_t buf_len = 128;
  int16_t buffer[buf_len * 2];  // 双声道缓冲
  ...
  while (remaining > 0) {
    uint32_t chunk = min(remaining, buf_len);
    for (uint32_t i = 0; i < chunk; i++) {
      int16_t sample = (sin(phase) > 0) ? amplitude : -amplitude;
      buffer[i * 2] = sample;      // 左声道
      buffer[i * 2 + 1] = sample;  // 右声道
      phase += step;
    }
    size_t bytes_written = 0;
    i2s_write(I2S_PORT, buffer, chunk * sizeof(int16_t) * 2, &bytes_written, portMAX_DELAY);
    remaining -= chunk;
  }
}
